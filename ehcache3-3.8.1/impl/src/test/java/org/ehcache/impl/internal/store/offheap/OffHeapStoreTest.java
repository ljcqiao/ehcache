...
public class OffHeapStoreTest extends AbstractOffHeapStoreTest {
  ...
  protected OffHeapStore<String, String> createAndInitStore(TimeSource timeSource, ExpiryPolicy<? super String, ? super String> expiry) {
	...
	/*
	 * @Override protected OffHeapStore<String, byte[]>
	 ...
	 * serializationProvider.start(providerContaining()); ClassLoader classLoader =
	 * getClass().getClassLoader(); Serializer<String> keySerializer =
	 * serializationProvider.createValueSerializer(String.class, classLoader);
	 ...
	 * getClass().getClassLoader(), expiry, null, 0, keySerializer,
	 * valueSerializer); OffHeapStore<String, byte[]> offHeapStore = new
	 * OffHeapStore<String, byte[]>(storeConfiguration, timeSource, new
	 * TestStoreEventDispatcher<>(), MemoryUnit.MB .toBytes(1), new
	 ...
	 * OffHeapStore.Provider.init(offHeapStore); return offHeapStore; } catch
